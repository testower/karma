...
var fileCache = {
  ...
  readFile: function (filePath, callback) {
    if (fileCache._cache[filePath] === undefined) {
      fs.readFile(filePath, function (err, buffer) {
        ...
        fs.watchFile(filePath, function (curr, prev) {
          if (curr.mtime !== prev.mtime) {
            fileCache._cache[filePath] = curr
          }
        })
        fileCache._cache[filePath] = buffer
        callback(fileCache._cache[filePath])
      })
    } else {
      callback(fileCache._cache[filePath])
    }
...
var createPreprocessor = function (config, basePath, injector) {
  ...
  return function preprocess (file, done) {
    ...
    fileCache.readFile(file.originalPath, function (buffer) {
